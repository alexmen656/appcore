...
export default function BossJobs() {
  ...
  const filtered = jobs.filter((j) =>
    !search || j.name.includes(search) || j.id.includes(search)
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">pg-boss Jobs</h1>
          <p className="text-muted-foreground text-sm mt-1">Schedules und Job-Queues</p>
        </div>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {!loading && schedules.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center text-muted-foreground py-8">Keine Schedules gefunden</TableCell>
                    </TableRow>
                  )}
                  {schedules.map((s) => (
                    <TableRow key={s.name}>
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        {QUEUES.map((q) => (
          <Card
            key={q}
            className={`cursor-pointer transition-all hover:shadow-md ${selectedQueue === q ? "ring-2 ring-orange-500" : ""}`}
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {(selectedQueue || selectedState || search) && (
          <Button variant="ghost" size="sm" onClick={() => { setSelectedQueue(""); setSelectedState(""); setSearch(""); }}>
            Filter zurücksetzen
          </Button>
        )}
            ...
            <Table>
              ...
              <TableBody>
                ...
                {!loading && filtered.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center text-muted-foreground py-8">Keine Jobs gefunden</TableCell>
                  </TableRow>
                )}
                {filtered.map((job) => (
                  <>
                    ...
                    >
                      ...
                      <TableCell>
                        <span className={`text-xs px-2 py-0.5 rounded font-medium ${STATE_COLORS[job.state] ?? "bg-gray-100 text-gray-700"}`}>
                          {job.state}
                        </span>
                      </TableCell>
                      ...
                      <TableCell className="font-mono text-xs text-muted-foreground truncate max-w-24">{job.id.slice(0, 8)}…</TableCell>
                    </TableRow>
                    {expandedId === job.id && (
