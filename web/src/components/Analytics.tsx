...
interface DownloadsData {
  byDay: { date: string; downloads: number; updates: number; proceeds: number }[];
...
export default function Analytics({ addToast }: Props) {
  ...
  const { data: downloads, loading: dlLoading, refetch: refetchDownloads } =
    useApi<DownloadsData>(`/analytics/downloads?bundleId=${bundleId}&days=90`);
  const { data: reviews, loading: rvLoading, refetch: refetchReviews } =
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[13px] text-[#9ca3af] mt-0.5">
            ...
            {summary?.lastSyncAt && (
              <span className="ml-2">· Last synced {fmtDate(summary.lastSyncAt)}</span>
            )}
          </p>
        </div>
        <button
          onClick={handleSync}
          disabled={syncing}
          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-[6px] text-[13px] font-medium bg-[#ea0e2b] text-white hover:bg-[#c80b24] disabled:opacity-60 transition-colors"
        >
          {syncing ? (
            <>
      ...
      {!loading && !summary?.totalDownloads30d && reviews?.length === 0 && (
        <div className="mb-5 px-4 py-3.5 rounded-xl bg-amber-50 border border-amber-200 text-[13px] text-amber-800">
          ...
          <a href="/settings" className="underline font-medium">Vendor Number</a>{" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[#e5e7eb]">
            <div className="text-[15px] font-semibold text-[#1a1a2e]">Top Countries</div>
            <div className="text-[12px] text-[#9ca3af] mt-0.5">Downloads by country (all time)</div>
          </div>
          {(downloads?.byCountry ?? []).length === 0 ? (
            <div className="px-5 py-8 text-center text-[13px] text-[#9ca3af]">No data yet</div>
          ) : (
              ...
              <tbody>
                {(() => {
                  ...
                  return (downloads?.byCountry ?? []).slice(0, 10).map((r) => (
                    <tr key={r.country} className="hover:bg-[#f7f8fa] transition-colors">
                      <td className={TD}>
                        <span className="font-medium text-[#1a1a2e]">{r.country}</span>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          <div className="w-16 h-1.5 bg-[#f0f0f0] rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${total > 0 ? (r.downloads / total) * 100 : 0}%` }}
                            />
                          </div>
        ...
        {/* Rating distribution */}
        <div className="bg-white border border-[#e5e7eb] rounded-xl p-5">
          <div className="text-[15px] font-semibold text-[#1a1a2e] mb-1">Rating Distribution</div>
          <div className="text-[12px] text-[#9ca3af] mb-4">Based on synced reviews</div>
          {(reviews ?? []).length === 0 ? (
            <div className="py-8 text-center text-[13px] text-[#9ca3af]">No reviews yet</div>
          ) : (
              ...
              {[5, 4, 3, 2, 1].map((star) => {
                ...
                return (
                  <div key={star} className="flex items-center gap-3">
                    <span className="text-[13px] text-[#1a1a2e] w-5 text-right">{star}</span>
