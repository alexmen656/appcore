...
export default function SuggestionCard({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 mb-3">
        {s.currentValue && (
          <div className="bg-red-50/60 dark:bg-red-900/10 rounded-xl px-4 py-3 text-xs text-[#4b5563] dark:text-[#8b93a5]">
            <div
              className={`text-[10px] font-medium uppercase tracking-wide ${textMuted} mb-1`}
            >
              Current
            </div>
            {s.currentValue}
        ...
        <div
          className={`bg-emerald-50/60 dark:bg-emerald-900/10 rounded-xl px-4 py-3 text-xs ${textPrimary}`}
        >
          <div
            className={`text-[10px] font-medium uppercase tracking-wide ${textMuted} mb-1`}
          >
      ...
      {s.reasoning && (
        <div className={`text-xs ${textSecondary} leading-relaxed mb-3`}>
          {s.reasoning}
        </div>
      )}
      {s.status === "PENDING" && (
