...
export async function buildWithGym(
  ...
  );//— scheme: ${gymScheme ?? appName}, export: ${exportMethod}`,
  ...
  //logs.push(`[build] Gymfile written:\n${gymfileLines.join("\n")}`);
  ...
  try {
    ...
    throw new Error(
      `build exited with code ${gymErr.code ?? "unknown"}.`,
    );
  }
  ...
  //logs.push(`[build] IPA found at: ${ipaFile}`);
  ...
  logs.push(
    `[build] Binary saved (${(ipaSize / 1024 / 1024).toFixed(1)} MB)`,
  );
  await signingCleanup?.();
  return destIpa;
}
