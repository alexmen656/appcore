...
snapshotRouter.post("/snapshot", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const snapfilePath = path.join(fastlaneDir, "Snapfile");

    const scheme = gymScheme ?? appName;
    const effectiveDevices = devices?.length ? devices : DEFAULT_DEVICES;
    const effectiveLanguages = languages?.length
      ? languages
      : DEFAULT_LANGUAGES;

    logs.push(
      `Writing AppCore Snapfile — scheme: ${scheme}, devices: ${effectiveDevices.join(", ")}, languages: ${effectiveLanguages.join(", ")}`,
    );
    fs.writeFileSync(
      snapfilePath,
      [
    ...
    const findDescriptionFile = (dir: string): string | null => {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        if (entry.name.startsWith(".") || entry.name === "fastlane") continue;
        const full = path.join(dir, entry.name);
        if (entry.isDirectory()) {
          const found = findDescriptionFile(full);
    ...
    const descFile = findDescriptionFile(tmpDir);
    if (descFile) {
      try {
        const parsed = JSON.parse(fs.readFileSync(descFile, "utf8"));
        ...
        descriptions = parsed;
        logs.push(
          `Loaded ${Object.keys(descriptions).length} screenshot description(s) from ${path.relative(tmpDir, descFile)}`,
        );
      } catch {
        logs.push(
          `Warning: could not parse ${path.relative(tmpDir, descFile)}`,
        );
      }
    }
